...
export class PythonShell extends EventEmitter {
    ...
    constructor(scriptPath: string, options?: Options, stdoutSplitter: Transform = null, stderrSplitter: Transform = null) {
         ...
         */
        function resolve(type: any, val: string | Function) {
     ...
     static run(scriptPath: string, options?: Options): Promise<any[]> {
        return new Promise((resolve, reject) => {
            ...
            pyshell.on('message', function (message) {
                ...
                options.onPrint&&options.onPrint({ type: 'message', data: message });
                if(!options.noOutput) {
                    output.push(message);
                }
            }).end(function (err) {
                ...
                if(err){
                    ...
                    // options.onPrint&&options.onPrint({ type: 'end', error: err });
                    reject(err);
                }
                else {
                    // options.onPrint&&options.onPrint({type: 'end' });
                    resolve(output);
                }
            });
        });
